...
static void draw(Canvas* canvas, void* ctx) {
    ...
    if(s->in_menu) {
        ...
        canvas_draw_str(canvas, 2, 40, s->output_raw_ir ? "Raw IR ON" : "Raw IR OFF");
    } else {
        char buf[32];
...
if(!st.serial_handle) {
    furi_log_puts("IRSerial", "Failed to acquire UART");
    return -1;
}
furi_log_puts("IRSerial", "App started, UART acquired");
