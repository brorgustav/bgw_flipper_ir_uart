...
static void process_ir(FlameTunnelState *s, InfraredWorkerSignal *sig,
  ...
  if (s->log_to_file) {
    ...
    file_stream_open(file, "/ext/flame_tunnel.log", FSAM_WRITE,
                     FSOM_OPEN_ALWAYS);
...
static void draw(Canvas *canvas, void *ctx) {
  ...
  if (s->in_menu) {
    ...
    canvas_draw_str(canvas, 2, 40,
                    s->output_raw_ir ? "Raw IR ON" : "Raw IR OFF");
  } else {
    char buf[32];
...
static void input_cb(InputEvent *ev, void *ctx) {
  ...
  if (ev->type == InputTypePress) {
    if (ev->key == InputKeyBack)
      s->back_pressed_time = now;
    if (ev->key == InputKeyOk)
      s->ok_pressed_time = now;
    ...
    if (ev->key == InputKeyBack && now - s->back_pressed_time >= HOLD_TIME_MS) {
      ...
      if (ev->key == InputKeyLeft || ev->key == InputKeyRight) {
        ...
        s->output_raw_ir = !s->output_raw_ir;
      }
    }
  }
}
...
int32_t bgw_flipper_ir_serial_app(void *p) {
  ...
  FURI_LOG_E("IRSerial", "App started, UART acquired");
